...
def run_stoned(
        s: str, fp_type: str = 'ECFP4', num_samples: int = 2000,
        max_mutations: int = 2, min_mutations: int = 1, alphabet: Union[List[str], Set[str]] = None) -> Tuple[List[str], List[str]]:
...
def sample_space(origin_smiles: str, f: Union[Callable[[str, str], float], Callable[[List[str], List[str]], List[float]]], batched: bool = True, preset: str = 'medium', stoned_kwargs: Dict = None) -> List[Examples]:
...
def cf_explain(examples: List[Examples], nmols: int = 3) -> List[Examples]:
...
def rcf_explain(examples: List[Examples], delta: Union[float, Tuple[float, float]] = (-1, 1), nmols: int = 4) -> List[Examples]:
    '''From given :obj:`Examples`, find best counterfactuals using :ref:`readme_link:counterfactual generation`
