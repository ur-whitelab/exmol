...
def text_explain2_maccs_and_ecfp(
    examples: list[Example],
    num_descriptors = 10
) -> str:
    """Take an example and convert t-statistics into text explanations using both MACCS and ECFP descriptors

    :param examples: Output from :func:`sample_space`
    :param num_descriptors: Maximum number of descriptors to include in prompt
    """
    """Take an example and convert t-statistics into text explanations"""
    from importlib_resources import files
    import exmol.lime_data
    ...
    lime_explain(examples, descriptor_type='maccs')
    tstats_maccs = list(examples[0].descriptors.tstats)
    ...
    d_importance_maccs = dict(
        sorted(d_importance_maccs.items(), key=lambda item: abs(item[1][0]), reverse=True)
    )
    
    #Then get results for ECFP descriptors
    lime_explain(examples, descriptor_type='ecfp')
    tstats_ecfp = list(examples[0].descriptors.tstats)
    ...
    d_importance_ecfp = dict(
        sorted(d_importance_ecfp.items(), key=lambda item: abs(item[1][0]), reverse=True)
    )
    
    #Combine the two dictionaries
    d_importance_ecfp.update(d_importance_maccs) #combine both dictionaries using update function (https://www.geeksforgeeks.org/python-merging-two-dictionaries/) 
    d_importance = d_importance_ecfp
    d_importance = dict(
        sorted(d_importance.items(), key=lambda item: abs(item[1][0]), reverse=True)
    )
    ...
    # need to get base molecule for naming
    base_mol = [exmol.smi2mol(e.smiles) for e in examples if e.is_origin == True]
    if len(base_mol) > 1:
        multiple_bases = True
        bi = {}  # type: Dict[Any, Any]
        for b in base_mol:
        ...
        multiple_bases = False
        base_mol = exmol.smi2mol(examples[0].smiles)
        bi = {}  # type: Dict[Any, Any]
        AllChem.GetMorganFingerprint(base_mol, 3, bitInfo=bi)
    ...
    success = 0
    for i, (k, v) in enumerate(d_importance.items()):
        if(isinstance(k, int)): #ECFP descriptor type
            descriptor_type = "ecfp"
        else:
            descriptor_type = "maccs"
        if success == num_descriptors:
            break
        name = k
        if descriptor_type.lower() == "ecfp" or isinstance(name, int):
            # convert names
